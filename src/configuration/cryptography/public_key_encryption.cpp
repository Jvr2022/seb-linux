...
contracts::LoadStatus PublicKeyEncryption::decrypt(const QByteArray &data, QByteArray &decrypted, QSslCertificate &certificate) const
{
    ...
    EVP_PKEY *pkey = reinterpret_cast<EVP_PKEY*>(privateKey.handle());
    ...
    size_t outLen = 0;
    if (EVP_PKEY_decrypt(ctx, nullptr, &outLen, reinterpret_cast<const unsigned char*>(cipherData.constData()), cipherData.size()) <= 0) {
        EVP_PKEY_CTX_free(ctx);
        return contracts::LoadStatus::InvalidData;
    }

    decrypted.resize(static_cast<int>(outLen));
    if (EVP_PKEY_decrypt(ctx, reinterpret_cast<unsigned char*>(decrypted.data()), &outLen, reinterpret_cast<const unsigned char*>(cipherData.constData()), cipherData.size()) <= 0) {
        EVP_PKEY_CTX_free(ctx);
        return contracts::LoadStatus::InvalidData;
    }
